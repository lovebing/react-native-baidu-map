    ...
    if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
      ...
      if(poiList != null) {
        for (PoiInfo poiInfo : poiList) {
          ...
          poiMap.putString("uid", poiInfo.uid);
          poiMap.putString("name", poiInfo.name);
          poiMap.putString("address", poiInfo.address);
          poiMap.putDouble("latitude", poiInfo.location.latitude);
          poiMap.putDouble("longitude", poiInfo.location.longitude);
          poiMap.putString("city", poiInfo.city);

          poiArray.pushMap(poiMap);
        }
      }
      params.putArray("poiList", poiArray);
    }
