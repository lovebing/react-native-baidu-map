...
- (NSArray<NSString *> *)supportedEvents {
  return @[@"onGetGeoCodeResult", @"onGetReverseGeoCodeResult"];
}
...
- (void) onGetReverseGeoCodeResult:(BMKGeoCodeSearch *)searcher result:(BMKReverseGeoCodeResult *)result
  ...
  if (error == BMK_SEARCH_NO_ERROR) {
    ...
    NSMutableArray<NSDictionary*> *poiList = [[NSMutableArray alloc] initWithCapacity:20];
    [result.poiList enumerateObjectsUsingBlock:^(BMKPoiInfo *poiInfo, NSUInteger index, BOOL *stop) {
      [poiList addObject:@{@"uid": poiInfo.uid,
                           @"name": poiInfo.name,
                           @"latitude": [NSString stringWithFormat:@"%f",poiInfo.pt.latitude],
                           @"longitude": [NSString stringWithFormat:@"%f",poiInfo.pt.longitude],
                           @"city":poiInfo.city,
                           @"address":poiInfo.address}];
    }];
    body[@"poiList"] = poiList;
  }
  else {
    body[@"errcode"] = [NSString stringWithFormat:@"%d", error];
    body[@"errmsg"] = [self getSearchErrorInfo:error];
